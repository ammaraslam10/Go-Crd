<?php include('/../assets/inc/config.php'); $template['header_link'] = 'CREATE USER - ADMIN'; ?>
    ...
    <div class="row">
        ...
        if(isset($_POST['create_admin'])){
            ...
            if(!UserAlreadyExists($email)){
                $query = $db->query("INSERT user (email,contact,password,username,is_active,total_cards,date,admin_id) VALUES ('$email','$phone','$password','$username','1','0','$date','$_SESSION[id]')");

                $GetU = $db->query("SELECT * FROM user WHERE email='$email' and admin_id='$_SESSION[id]'");
                $gu = $GetU->fetch_assoc();
                    
                $file = '../uploads/default.jpg';
                $newfile = '../uploads/user/'.$gu[id].'.jpg';
                ...
                <form id="form-validation" action="" method="post" class="form-horizontal form-bordered" autocomplete="off">
                    ...
                    <div class="form-group form-actions">
                        <div class="col-md-8 col-md-offset-3">
                            <button type="submit" name="create_admin" class="btn btn-effect-ripple btn-success">Create User</button>
                        </div>
                    </div>
                </form>
