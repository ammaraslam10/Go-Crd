...
if($b == "add") {
        ...
        <div class="content mt-3">
           ...
           <div class="col-md-12">
			...
			if(isset($_POST['btn_add'])) {
				...
				$check = $db->query("SELECT * FROM pw_pages WHERE prefix='$prefix'");
				...
				elseif($check->num_rows>0) { echo error("This prefix is already used. Please choose another. "); }
					...
					$time = time();
					$insert = $db->query("INSERT pw_pages (title,prefix,content,created) VALUES ('$title','$prefix','$content','$time')");
	...
	$id = protect($_GET['id']);
	$query = $db->query("SELECT * FROM pw_pages WHERE id='$id'");
           ...
           <div class="col-md-12">
			...
			if(isset($_POST['btn_save'])) {
				...
				elseif($row['prefix'] !== $prefix && $check->num_rows>0) { echo error("This prefix is already used. Please choose another. "); }
					...
					$link = '<a href="'.$page.'" target="_blank">'.$page.'</a>';
					$time = time();
					$update = $db->query("UPDATE pw_pages SET title='$title',prefix='$prefix',content='$content',updated='$time' WHERE id='$row[id]'");
					$query = $db->query("SELECT * FROM pw_pages WHERE id='$id'");
					$row = $query->fetch_assoc();
        ...
        <div class="content mt-3">
           ...
           <div class="col-md-12">
			...
			if(isset($_GET['confirm'])) {
				$delete = $db->query("DELETE FROM pw_pages WHERE id='$row[id]'");
